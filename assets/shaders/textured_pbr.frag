...
void main()
{
    ...
    vec3 surfAlbedo = albedo * texture(albedoTex,TexCoords).xyz;

    vec3 F0 = vec3(0.04); 
    F0 = mix(F0, surfAlbedo, metallic);
    ...
    for(int i = 0; i < 4; ++i) 
    {
        ...
        Lo += outRadiance(L,V,N,F0,surfAlbedo,inRadiance);
    }
    ...
    vec3 ambient = vec3(0.03) * surfAlbedo * ao;
