...
void main()
{
	...
	vec4 diffuse_albedo = fcolor;					// This is the vertex color, used to handle the colormode change
	...
	vec3 diffuse = lightColors[0] * max(dot(N, L), 0.0) * diffuse_albedo.xyz;

	// Calculate the specular component using Phong specular reflection
	vec3 V = normalize(-fposition);	
	vec3 R = reflect(-L, N);
	vec3 specular = lightColors[0] * pow(max(dot(R, V), 0.0), shininess) * specular_albedo;
